        ...
        try {
          ...
          if (!installationId) {
            const hardwareIdParts = [
              ...
              // Add a random component to differentiate identical hardware
              Math.random().toString().slice(2)
            ];
            const hardwareFingerprint = hardwareIdParts.join("_");
            const randomBytes = new Uint8Array(16);
            crypto.getRandomValues(randomBytes);
            const randomPart = Array.from(randomBytes).map((b) => b.toString(16).padStart(2, "0")).join("");
            installationId = hardwareFingerprint + "-" + randomPart;
            localStorage.setItem("installation_id", installationId);
            console.log("Created new installation ID:", installationId.substring(0, 10) + "...");
          } else {
            console.log("Using existing installation ID:", installationId.substring(0, 10) + "...");
          }
