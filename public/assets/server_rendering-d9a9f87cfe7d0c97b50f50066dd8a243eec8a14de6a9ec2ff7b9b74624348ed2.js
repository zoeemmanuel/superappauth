(() => {
  ...
  var init_createLucideIcon = __esm({
    "node_modules/lucide-react/dist/esm/createLucideIcon.js"() {
      ...
      toKebabCase = (string) => string.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase().trim();
  ...
  var require_deviceKey = __commonJS({
    "app/javascript/utils/deviceKey.js"(exports, module) {
      ...
      var createInstallationId = () => {
        const platform = navigator.platform || "unknown";
        const hardwareConcurrency = navigator.hardwareConcurrency || "unknown";
        const deviceMemory = navigator.deviceMemory || "unknown";
        const userAgent = navigator.userAgent;
        const osMatches = userAgent.match(/Windows NT (\d+\.\d+)|Mac OS X (\d+[._]\d+)|Android (\d+\.\d+)|iOS (\d+\.\d+)/i);
        const osVersion = osMatches ? osMatches[1] || osMatches[2] || osMatches[3] || osMatches[4] || "" : "unknown";
        const deviceModel = userAgent.match(/\(([^;]+);/i)?.[1] || "unknown";
        const hardwareId = `${platform}_${hardwareConcurrency}_${deviceMemory}_${osVersion}`;
        let deviceUuid = localStorage.getItem("device_uuid");
        if (!deviceUuid) {
          const array = new Uint8Array(8);
          window.crypto.getRandomValues(array);
          deviceUuid = Array.from(array, (byte) => byte.toString(16).padStart(2, "0")).join("");
          localStorage.setItem("device_uuid", deviceUuid);
          console.log("Created new device UUID for installation ID:", deviceUuid);
        }
        const screenInfo = `${window.screen.width}x${window.screen.height}x${window.screen.colorDepth}`;
        localStorage.setItem("hw_platform", platform);
        localStorage.setItem("hw_cores", hardwareConcurrency.toString());
        localStorage.setItem("hw_memory", deviceMemory.toString());
        localStorage.setItem("hw_osversion", osVersion);
        localStorage.setItem("hw_screeninfo", screenInfo);
        return `${hardwareId}_${deviceUuid}`;
      };
      var generateDeviceFingerprint = async () => {
        try {
          ...
          if (!installationId) {
            ...
            console.log("Created new installation ID:", installationId);
          } else {
          ...
          const fingerComponents = {
            // Most important for cross-browser matching:
            installationId,
            platform,
            hardwareConcurrency,
            deviceMemory,
            timeZone,
            language,
            // Screen info (treated as secondary)
            screenInfo,
            // These components may vary between browsers but are still useful:
            userAgent: navigator.userAgent,
            // Optional browser-specific components (used with lower weight):
            canvas: canvasFingerprint,
            webgl: webglFingerprint,
            audio: audioFingerprint
          };
          localStorage.setItem("fingerprint_components", JSON.stringify(fingerComponents));
          const priorityComponents = [
            // Installation ID has the highest priority (60% of the fingerprint)
            installationId,
            // Hardware components have medium priority (30% of the fingerprint)
            platform,
            hardwareConcurrency.toString(),
            deviceMemory.toString(),
            // Other components have lower priority (10% of the fingerprint)
            timeZone,
            language
          ];
          const combinedInfo = priorityComponents.join("|");
          console.log(
            "Combined fingerprint elements:",
            combinedInfo.substring(0, 50) + "..."
          );
          const encoder = new TextEncoder();
          const data = encoder.encode(combinedInfo);
          const hashBuffer = await crypto.subtle.digest("SHA-256", data);
          const deviceFingerprint = Array.from(new Uint8Array(hashBuffer)).map((b) => b.toString(16).padStart(2, "0")).join("");
          console.log("Generated device fingerprint:", deviceFingerprint.substring(0, 10) + "...");
          localStorage.setItem("device_fingerprint", deviceFingerprint);
          localStorage.setItem("fingerprint_version", "3");
          return deviceFingerprint;
        } catch (err) {
          console.error("Error generating fingerprint:", err);
          try {
            const basicInfo = `${navigator.platform}|${navigator.hardwareConcurrency}|${navigator.deviceMemory}`;
            console.log("Using basic fingerprint fallback:", basicInfo);
            const encoder = new TextEncoder();
            const data = encoder.encode(basicInfo);
            const hashBuffer = await crypto.subtle.digest("SHA-256", data);
            const basicFingerprint = Array.from(new Uint8Array(hashBuffer)).map((b) => b.toString(16).padStart(2, "0")).join("");
            localStorage.setItem("device_fingerprint", basicFingerprint);
            localStorage.setItem("fingerprint_version", "0");
            return basicFingerprint;
          } catch (e) {
            console.error("Basic fingerprinting failed:", e);
            return null;
          }
        }
      };
      var getFingerprintComponents2 = () => {
        try {
          const componentsString = localStorage.getItem("fingerprint_components");
          if (componentsString) {
            const components = JSON.parse(componentsString);
            if (components && components.installationId) {
              console.log("Sending installation ID:", components.installationId);
            } else {
              console.warn("No installation ID found in components");
            }
            return components;
          }
        } catch (e) {
          console.error("Error parsing fingerprint components:", e);
        }
        return null;
      };
      var generateCanvasFingerprint = async () => {
        try {
          ...
          ctx.fillStyle = "#069";
          ctx.fillText("SuperApp \u{1F44B}\u{1F30D}", 4, 4);
          ctx.globalCompositeOperation = "multiply";
          ctx.fillStyle = "rgb(255,0,255)";
          ctx.beginPath();
          ctx.arc(50, 25, 15, 0, Math.PI * 2, true);
          ctx.closePath();
          ctx.fill();
          const dataURL = canvas.toDataURL();
          return dataURL.substr(-64);
        } catch (e) {
          console.warn("Canvas fingerprinting not supported:", e);
          return "canvas-not-supported";
        }
      };
      var generateWebGLFingerprint = async () => {
        try {
          const canvas = document.createElement("canvas");
          ...
          const params = paramNames.map((name) => {
            try {
              return `${name}:${gl.getParameter(gl[name])}`;
            } catch (e) {
              return `${name}:null`;
            }
          }).join("|");
      ...
      var generateAudioFingerprint = async () => {
        try {
        ...
        try {
          ...
          if (existingFingerprint) {
            console.log(
              ...
              existingFingerprint.substring(0, 10) + "..."
            );
            return existingFingerprint;
          }
          return generateDeviceFingerprint();
        } catch (err) {
          console.error("Error retrieving/generating device fingerprint:", err);
          return null;
        }
      };
      ...
      module.exports = {
        generateDeviceKey,
        getStoredDeviceKey: getStoredDeviceKey2,
        storeDeviceKey,
        ...
        // New exports for component-based matching
        getFingerprintComponents: getFingerprintComponents2
      };
    }
  });

  // app/javascript/config/axios.js
  var import_deviceKey, instance, axios_default2;
  var init_axios3 = __esm({
    "app/javascript/config/axios.js"() {
      init_axios2();
      import_deviceKey = __toESM(require_deviceKey());
      instance = axios_default.create({
      ...
      instance.interceptors.request.use(async (config) => {
        try {
          ...
          const deviceFingerprint = await (0, import_deviceKey.getDeviceFingerprint)();
          if (deviceFingerprint) {
            console.log("Setting request header X-Device-Fingerprint:", deviceFingerprint.substring(0, 10) + "...");
            config.headers["X-Device-Fingerprint"] = deviceFingerprint;
          }
          const fingerprintComponents = (0, import_deviceKey.getFingerprintComponents)();
          if (fingerprintComponents) {
            console.log(
              "Setting request header X-Fingerprint-Components with installation ID:",
              fingerprintComponents.installationId ? fingerprintComponents.installationId.substring(0, 10) + "..." : "missing"
            );
            config.headers["X-Fingerprint-Components"] = JSON.stringify(fingerprintComponents);
          }
  ...
  var require_UnifiedLogin = __commonJS({
    "app/javascript/components/auth/UnifiedLogin.jsx"(exports, module) {
      ...
      var UnifiedLogin2 = () => {
        const [flowState, setFlowState] = (0, import_react3.useState)("checking");
        ...
        const [handle, setHandle] = (0, import_react3.useState)("");
        const [error, setError] = (0, import_react3.useState)("");
        const [phone, setPhone] = (0, import_react3.useState)("");
        const [isQuickVerification, setIsQuickVerification] = (0, import_react3.useState)(false);
        const [countryCode, setCountryCode] = (0, import_react3.useState)("+44");
        const [phoneNumber, setPhoneNumber] = (0, import_react3.useState)("");
        const [showCountrySelect, setShowCountrySelect] = (0, import_react3.useState)(false);
        const [isLoading, setIsLoading] = (0, import_react3.useState)(false);
        const [redirectAttempts, setRedirectAttempts] = (0, import_react3.useState)(0);
        const [welcomeMessage, setWelcomeMessage] = (0, import_react3.useState)("");
        const [loginMethod, setLoginMethod] = (0, import_react3.useState)("handle");
        const [existingUserData, setExistingUserData] = (0, import_react3.useState)(null);
        const [autoSubmit, setAutoSubmit] = (0, import_react3.useState)(true);
        const [deviceNotRegistered, setDeviceNotRegistered] = (0, import_react3.useState)(false);
        const [suggestedHandles, setSuggestedHandles] = (0, import_react3.useState)([]);
        const [showExistingAccountAlert, setShowExistingAccountAlert] = (0, import_react3.useState)(false);
        const [accountType, setAccountType] = (0, import_react3.useState)(null);
        const [existingHandle, setExistingHandle] = (0, import_react3.useState)("");
        const [isHandleFirst, setIsHandleFirst] = (0, import_react3.useState)(false);
        const [customHandle, setCustomHandle] = (0, import_react3.useState)("");
        const verificationInputRef = (0, import_react3.useRef)(null);
        const yesButtonRef = (0, import_react3.useRef)(null);
        const noButtonRef = (0, import_react3.useRef)(null);
        const MAX_REDIRECT_ATTEMPTS = 3;
        const handleRegisterDevice = (0, import_react3.useCallback)(async () => {
        ...
        const handleNotMyAccount = (0, import_react3.useCallback)(() => {
          setIsLoading(true);
          setError("");
        ...
        const checkDevice = async () => {
          ...
          try {
            ...
            if (deviceFingerprint) {
              console.log("Device fingerprint generated:", deviceFingerprint.substring(0, 10) + "...");
            }
        ...
        const handleHandleSubmit = async () => {
          ...
          try {
            ...
            const deviceKey = await (0, import_deviceKey2.getStoredDeviceKey)();
            const checkResponse = await axios_default2.get(`check_handle?handle=${encodeURIComponent(handle)}`);
            ...
            if (phone) {
              const response = await axios_default2.post("verify_code", {
                code: verificationCode,
                handle,
                // Important: Pass the handle for createHandle flow
                phone,
                auth: {
                  code: verificationCode,
                  handle,
        ...
        const renderLoadingState = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex flex-col items-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-spin rounded-full h-12 w-12 border-t-2 border-teal-500 mb-4" }), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-white text-lg" }, "Connecting to SuperApp...")));
        const renderDeviceNotRegistered = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md space-y-6 text-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "mb-6 flex justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-16 h-16 rounded-full bg-gray-800 flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement(X, { className: "w-8 h-8 text-gray-400" }))), /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-2xl font-bold" }, "Device Not Registered"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, "This device is not linked to any SuperApp account"), /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-pulse flex justify-center mt-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-8 h-8 rounded-full border-t-2 border-teal-500 animate-spin" }))));
        const renderDeviceRegistered = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md space-y-6 text-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "mb-6 flex justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-16 h-16 rounded-full bg-teal-500 flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement(Check, { className: "w-8 h-8 text-white" }))), /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-2xl font-bold" }, "Device Recognized"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, "Redirecting you to dashboard..."), /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-pulse flex justify-center mt-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-8 h-8 rounded-full border-t-2 border-teal-500 animate-spin" }))));
        const renderLoginSuccess = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md space-y-6 text-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "mb-6 flex justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-16 h-16 rounded-full bg-teal-500 flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement(Check, { className: "w-8 h-8 text-white" }))), /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-2xl font-bold" }, "Login Successful"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, "Redirecting you to dashboard..."), /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-pulse flex justify-center mt-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-8 h-8 rounded-full border-t-2 border-teal-500 animate-spin" }))));
        const renderVerificationStep = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "mb-8 flex justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-16 h-16 rounded-full bg-teal-500 flex items-center justify-center text-xl font-bold" }, handle ? handle[0].toUpperCase() : "S")), /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-center space-y-2 mb-8" }, /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-2xl font-bold" }, welcomeMessage || (isQuickVerification ? `Welcome back, ${handle}` : "Verify your number")), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, isQuickVerification ? /* @__PURE__ */ import_react3.default.createElement(import_react3.default.Fragment, null, "Please enter the code sent to ", phone, " to confirm this is your account") : `Please enter the code sent to ${phone} to confirm this is your account`)), /* @__PURE__ */ import_react3.default.createElement("div", { className: "mb-8" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex justify-center gap-3" }, [...Array(6)].map((_, i) => /* @__PURE__ */ import_react3.default.createElement(
          "div",
          {
            key: i,
        ...
        )), /* @__PURE__ */ import_react3.default.createElement("div", { className: "grid grid-cols-3 gap-6 mb-8" }, [1, 2, 3, 4, 5, 6, 7, 8, 9, "", 0, "\u232B"].map((num, i) => /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
            key: i,
            onClick: () => {
          ...
          num
        ))), error && /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-red-500 text-center mb-4 flex items-center justify-center p-3 bg-red-500 bg-opacity-10 rounded-lg" }, /* @__PURE__ */ import_react3.default.createElement("span", null, error), /* @__PURE__ */ import_react3.default.createElement("button", { onClick: () => setError(""), className: "ml-2" }, /* @__PURE__ */ import_react3.default.createElement(X, { className: "w-4 h-4" }))), isLoading && /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-center mb-4 text-gray-400" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "inline-block animate-spin rounded-full h-5 w-5 border-b-2 border-teal-500 mr-2" }), "Verifying..."), /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex justify-center mt-4" }, /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
        ...
        const renderLoginOptions = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md space-y-8" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-center" }, /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-2xl font-bold mb-2" }, "Welcome to SuperApp"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, deviceNotRegistered ? "This device is not registered. Please login with an existing account or create a new one." : "Login or register to continue")), showExistingAccountAlert && /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex flex-col items-center text-center space-y-6 py-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-16 h-16 rounded-full bg-teal-500 flex items-center justify-center" }, accountType === "handle" ? /* @__PURE__ */ import_react3.default.createElement("span", { className: "text-white text-2xl" }, "@") : /* @__PURE__ */ import_react3.default.createElement(User, { className: "w-8 h-8 text-white" })), /* @__PURE__ */ import_react3.default.createElement("div", { className: "space-y-2" }, /* @__PURE__ */ import_react3.default.createElement("h2", { className: "text-2xl font-bold" }, accountType === "handle" ? "This Handle Is Already Registered" : "Phone Number Already Registered"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, accountType === "handle" ? `The handle ${existingHandle || identifier} is already in use. Are you the owner of this account or would you like to choose a different handle?` : `This phone number is associated with an existing account. If this is your account, you can sign in. Otherwise, please use a different number.`)), /* @__PURE__ */ import_react3.default.createElement("div", { className: "space-y-3 w-full", id: "account-exists-buttons" }, /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
            id: "yes-account-btn",
          ...
          isLoading ? /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" }), "Verifying...") : accountType === "handle" ? "Sign In As Owner" : "Sign In To Existing Account"
        ), /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
            id: "no-account-btn",
        ...
        ))), !showExistingAccountAlert && /* @__PURE__ */ import_react3.default.createElement(import_react3.default.Fragment, null, /* @__PURE__ */ import_react3.default.createElement("div", { className: "bg-gray-800 p-1 rounded-lg flex mb-6" }, /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
          ...
          /* @__PURE__ */ import_react3.default.createElement(User, { size: 18 }),
          /* @__PURE__ */ import_react3.default.createElement("span", null, "Handle")
        ), /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
          ...
          /* @__PURE__ */ import_react3.default.createElement(Phone, { size: 18 }),
          /* @__PURE__ */ import_react3.default.createElement("span", null, "Phone")
        )), /* @__PURE__ */ import_react3.default.createElement("div", { className: "relative" }, loginMethod === "phone" ? /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex" }, /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
          ...
          countryCode,
          " ",
          /* @__PURE__ */ import_react3.default.createElement(ChevronDown, { size: 16, className: "text-gray-400" })
        ), /* @__PURE__ */ import_react3.default.createElement(
          "input",
          {
            type: "tel",
        ...
        )) : /* @__PURE__ */ import_react3.default.createElement("div", { className: "relative" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none" }, /* @__PURE__ */ import_react3.default.createElement("span", { className: "text-gray-400" }, "@")), /* @__PURE__ */ import_react3.default.createElement(
          "input",
          {
            type: "text",
        ...
        )), showCountrySelect && /* @__PURE__ */ import_react3.default.createElement("div", { className: "absolute mt-2 w-40 bg-gray-800 rounded-lg shadow-lg z-10 overflow-hidden" }, /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
            onClick: () => {
              setCountryCode("+44");
          ...
          isLoading ? /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" }), "Processing...") : /* @__PURE__ */ import_react3.default.createElement("span", { className: "flex items-center" }, "Continue ", /* @__PURE__ */ import_react3.default.createElement(ArrowRight, { size: 18, className: "ml-2" }))
        )), error && !showExistingAccountAlert && /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-red-500 text-center flex items-center justify-center p-3 bg-red-500 bg-opacity-10 rounded-lg" }, /* @__PURE__ */ import_react3.default.createElement(Lock, { size: 16, className: "mr-2" }), /* @__PURE__ */ import_react3.default.createElement("span", null, error), /* @__PURE__ */ import_react3.default.createElement("button", { onClick: () => setError(""), className: "ml-2" }, /* @__PURE__ */ import_react3.default.createElement(X, { className: "w-4 h-4" })))));
        const renderHandleEntry = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md space-y-8" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-center" }, /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-2xl font-bold mb-2" }, "Create your handle"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, "Choose a unique username to identify yourself")), /* @__PURE__ */ import_react3.default.createElement("div", { className: "relative" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none" }, /* @__PURE__ */ import_react3.default.createElement("span", { className: "text-gray-400" }, "@")), /* @__PURE__ */ import_react3.default.createElement(
          "input",
          {
            type: "text",
            value: identifier.replace("@", ""),
        ...
        const renderPhoneEntry = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md space-y-8" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-center" }, /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-2xl font-bold mb-2" }, "Enter your phone"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, "We'll send a verification code to verify your number")), /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex mb-2" }, /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
            onClick: () => setShowCountrySelect(!showCountrySelect),
            className: "bg-gray-800 rounded-l-lg py-4 px-4 flex items-center gap-2 border-r border-gray-700",
          ...
          isLoading ? /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" }), "Sending code...") : /* @__PURE__ */ import_react3.default.createElement("span", { className: "flex items-center" }, "Send Verification Code ", /* @__PURE__ */ import_react3.default.createElement(ArrowRight, { size: 18, className: "ml-2" }))
        ), /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
        ...
        const renderCreateHandle = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md space-y-8" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-center" }, /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-2xl font-bold mb-2" }, "Create your handle"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400" }, "This will be your unique identifier")), /* @__PURE__ */ import_react3.default.createElement("div", null, /* @__PURE__ */ import_react3.default.createElement("div", { className: "relative" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none" }, /* @__PURE__ */ import_react3.default.createElement("span", { className: "text-gray-400" }, "@")), /* @__PURE__ */ import_react3.default.createElement(
          "input",
          {
            type: "text",
        ...
        )), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-xs text-gray-400 mt-2 pl-2" }, "Handle must start with @ and contain only letters, numbers, and underscores")), /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
          ...
          isLoading ? /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" }), "Setting up your account...") : /* @__PURE__ */ import_react3.default.createElement("span", { className: "flex items-center" }, "Create Handle ", /* @__PURE__ */ import_react3.default.createElement(ArrowRight, { size: 18, className: "ml-2" }))
        ), error && /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-red-500 text-center flex items-center justify-center p-3 bg-red-500 bg-opacity-10 rounded-lg" }, /* @__PURE__ */ import_react3.default.createElement("span", null, error), /* @__PURE__ */ import_react3.default.createElement("button", { onClick: () => setError(""), className: "ml-2" }, /* @__PURE__ */ import_react3.default.createElement(X, { className: "w-4 h-4" })))));
        const renderHandleSuggestions = () => /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex items-center mb-6" }, /* @__PURE__ */ import_react3.default.createElement(
          "button",
          {
            onClick: () => setFlowState("loginOptions"),
            className: "text-gray-400 hover:text-white transition-colors"
          },
          /* @__PURE__ */ import_react3.default.createElement(ArrowLeft, { size: 20 })
        ), /* @__PURE__ */ import_react3.default.createElement("h1", { className: "text-xl font-medium ml-4" }, "Choose a handle")), /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex justify-center mb-6" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-16 h-16 rounded-full bg-teal-500 flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement("span", { className: "text-2xl font-bold" }, "@"))), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-center text-gray-400 mb-6" }, "The handle you selected is already taken. Here are some available alternatives:"), /* @__PURE__ */ import_react3.default.createElement("div", { className: "space-y-2 mb-6" }, suggestedHandles.map((handle2, index) => /* @__PURE__ */ import_react3.default.createElement(
          "div",
          {
            key: index,
            onClick: () => {
          ...
          /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex items-center justify-between" }, /* @__PURE__ */ import_react3.default.createElement("span", { className: "font-medium" }, handle2), /* @__PURE__ */ import_react3.default.createElement(ArrowRight, { size: 16, className: "text-teal-500" }))
        ))), /* @__PURE__ */ import_react3.default.createElement("div", { className: "mt-8" }, /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-sm text-gray-400 mb-2" }, "Or create your own handle:"), /* @__PURE__ */ import_react3.default.createElement("div", { className: "relative" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none" }, /* @__PURE__ */ import_react3.default.createElement("span", { className: "text-gray-400" }, "@")), /* @__PURE__ */ import_react3.default.createElement(
          "input",
          {
            type: "text",
            value: customHandle || "",
        ...
        const renderDeviceRegistration = () => {
          ...
          return /* @__PURE__ */ import_react3.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-full max-w-md space-y-6" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "mb-8 flex justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "w-16 h-16 rounded-full bg-teal-500 flex items-center justify-center text-xl font-bold" }, existingUserData?.handle ? existingUserData.handle[0].toUpperCase() : identifier.startsWith("@") ? identifier[1].toUpperCase() : "S")), /* @__PURE__ */ import_react3.default.createElement("div", { className: "text-center" }, /* @__PURE__ */ import_react3.default.createElement("h2", { className: "text-2xl font-bold mb-2" }, isPhoneFlow ? "Phone Number Already Registered" : "This Handle Is Already Registered"), /* @__PURE__ */ import_react3.default.createElement("p", { className: "text-gray-400 mb-6" }, isPhoneFlow ? `This phone number is associated with an existing account. If this is your account, you can sign in. Otherwise, please use a different number.` : `The handle ${existingUserData?.masked_handle || maskHandle(identifier)} is already in use. Are you the owner of this account or would you like to choose a different handle?`)), /* @__PURE__ */ import_react3.default.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ import_react3.default.createElement(
            "button",
            {
              ref: yesButtonRef,
              onClick: handleRegisterDevice,
            ...
            isLoading ? /* @__PURE__ */ import_react3.default.createElement("div", { className: "flex items-center justify-center" }, /* @__PURE__ */ import_react3.default.createElement("div", { className: "animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" }), "Processing...") : /* @__PURE__ */ import_react3.default.createElement("span", { className: "flex items-center" }, isPhoneFlow ? "Sign In To Existing Account" : "Sign In As Owner", " ", /* @__PURE__ */ import_react3.default.createElement(ArrowRight, { size: 18, className: "ml-2" }))
          ), /* @__PURE__ */ import_react3.default.createElement(
            "button",
            {
              ref: noButtonRef,
  ...
  global.React = import_react2.default;
