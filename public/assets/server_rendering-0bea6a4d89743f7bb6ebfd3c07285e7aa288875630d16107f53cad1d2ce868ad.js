  ...
  var require_deviceKey = __commonJS({
    "app/javascript/utils/deviceKey.js"(exports, module) {
      ...
      var getDeviceFingerprint2 = async () => {
        ...
        return await getStoredDeviceKey3();
      };
      ...
      if (loginMethod === "handle") {
        ...
        try {
          ...
          if (checkResponse.data.exists && !checkResponse.data.is_your_device) {
            ...
            const hasPinAvailable = await checkPinAvailability(identifier);
            ...
            setPinAvailable(hasPinAvailable);
            setIsLoading(false);
            return;
          }
          ...
          if (response.data.status === "verification_needed") {
            ...
            if (response.data.pin_available) {
              ...
              await checkPinAvailability(response.data.handle);
            }
            return;
            ...
            const hasPinAvailable = await checkPinAvailability(response.data.handle || identifier);
            sessionStorage.setItem("account_already_exists", JSON.stringify({
              type: "handle",
          ...
          if (err.response?.data?.status === "handle_not_found") {
            ...
            const hasPinAvailable = await checkPinAvailability(err.response?.data?.handle || identifier);
            sessionStorage.setItem("account_already_exists", JSON.stringify({
              type: "handle",
        ...
        try {
          ...
          if (response.data.status === "verification_needed") {
            ...
            const hasPinAvailable = await checkPinAvailability(response.data.handle);
            sessionStorage.setItem("account_already_exists", JSON.stringify({
              type: "phone",
              phone: identifier,
          ...
          if (err.response?.data?.status === "phone_not_found") {
            ...
            const hasPinAvailable = await checkPinAvailability(err.response?.data?.handle);
            sessionStorage.setItem("account_already_exists", JSON.stringify({
              type: "phone",
              phone: identifier,
