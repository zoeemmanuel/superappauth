  ...
  var UnifiedLogin = () => {
    ...
    const handleInitialSubmit = async () => {
      ...
      try {
        ...
        if (storedHeader) {
          try {
            existingHeader = JSON.parse(storedHeader);
            if (existingHeader.userGuid) {
              deviceHeaderData.userGuid = existingHeader.userGuid;
            }
            if (existingHeader.userHandle) {
              deviceHeaderData.userHandle = existingHeader.userHandle;
            }
          } catch (e) {
        ...
        if (loginMethod === "handle" && identifier.startsWith("@")) {
          ...
          try {
            const fastResponse = await axios_default2.post("api/v1/fast_authenticate", {
              identifier
            }, {
              headers: {
        ...
        if (loginMethod === "handle") {
          ...
          try {
            ...
            const checkResponse = await axios_default2.get(`api/v1/check_handle?handle=${encodeURIComponent(identifier)}`, {
              headers: {
                "X-Device-Key": deviceKey,
                "X-Device-Header": JSON.stringify(deviceHeaderData)
              }
            ...
            if (checkResponse.data.exists) {
              ...
              if (isRecognizedDevice && deviceConfidence === "high") {
                ...
                const fastLoginResponse = await axios_default2.post("api/v1/fast_authenticate", {
                  identifier
                }, {
                  headers: {
                    "X-CSRF-Token": window.csrfToken,
            ...
            const response = await axios_default2.post("api/v1/verify_login", {
              identifier,
              auth: { identifier }
            }, {
              headers: {
          ...
          try {
            ...
            const checkResponse = await axios_default2.get(`api/v1/check_phone?phone=${encodeURIComponent(identifier)}`, {
              headers: {
                "X-Device-Key": deviceKey,
                "X-Device-Header": JSON.stringify(deviceHeaderData)
              }
