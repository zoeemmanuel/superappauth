(() => {
  ...
  var UnifiedLogin = () => {
    ...
    const handleInitialSubmit = async () => {
      ...
      try {
        ...
        if (loginMethod === "handle" && identifier.startsWith("@")) {
          console.log("Handling login for handle:", identifier);
          if ((0, import_deviceKey2.isWebAuthnSupported)()) {
            try {
              console.log("WebAuthn is supported, attempting verification");
              const webAuthnResult = await (0, import_deviceKey2.verifyWebAuthnCredential)(identifier);
              if (webAuthnResult.success) {
                console.log("WebAuthn verification successful!");
                localStorage.setItem("authenticated_user", "true");
                if (webAuthnResult.data && webAuthnResult.data.redirect_to) {
                  window.location.href = webAuthnResult.data.redirect_to;
                  clearTimeout(loadingTimeout);
                  return;
                } else {
                  window.location.href = "/dashboard";
              ...
              console.log("WebAuthn verification failed, continuing with regular flow");
            } catch (webAuthnError) {
              console.error("Error during WebAuthn verification:", webAuthnError);
            }
          } else {
            console.log("WebAuthn is not supported on this browser, skipping verification");
          }
          ...
          if (loginMethod === "handle") {
            ...
            try {
              ...
              console.log("Phone pre-check request headers:", Object.keys(requestHeaders));
              ...
              if (response.data.status === "authenticated") {
                ...
                console.warn("Unknown phone check status:", response.data.status);
  ...
  `;
      ...
      try {
        ...
        if (response.data.status === "authenticated") {
          ...
          if ((0, import_deviceKey2.isWebAuthnSupported)()) {
            ...
            console.log("WebAuthn not supported on this browser, skipping registration");
          }
          localStorage.setItem("authenticated_user", "true");
          setFlowState("loginSuccess");
    ...
    ), console.log("PIN availability check:", pinAvailable, existingUserData?.pin_available), (pinAvailable || existingUserData && existingUserData.pin_available) && /* @__PURE__ */ import_react2.default.createElement(
      "button",
      {
        id: "pin-signin-btn",
        onClick: () => {
          console.log("PIN button clicked, setting showPinEntry to true");
          setShowPinEntry(true);
        },
