(() => {
  ...
  var require_deviceKey = __commonJS({
    "app/javascript/utils/deviceKey.js"(exports, module) {
      ...
      var detectBrowserFamily = (userAgent) => {
      ...
      var generateDeviceHeader3 = (deviceId, userGuid, userHandle) => {
        if (!deviceId || !userGuid || !userHandle) {
        ...
        try {
          ...
          const headerData = {
            ...
            userGuid,
            userHandle,
            // FIXED: No longer using empty string fallback
            timestamp: Date.now(),
      ...
      var getDeviceHeader3 = () => {
        try {
          ...
          console.log(
            "Found valid device header in localStorage for cross-browser recognition:",
  ...
  var UnifiedLogin = () => {
    ...
    const handleRegisterDevice = (0, import_react2.useCallback)(async () => {
      ...
      try {
        ...
        const existingHeader = getExistingDeviceHeader();
        const requestHeaders = {
          "X-CSRF-Token": csrfToken
        };
        if (existingHeader) {
          console.log("Using existing device header for device registration:", {
            deviceId: existingHeader.deviceId.substring(0, 10) + "...",
            userGuid: existingHeader.userGuid,
            userHandle: existingHeader.userHandle
          });
          requestHeaders["X-Device-Header"] = JSON.stringify(existingHeader);
        } else {
          const deviceCharacteristics = {
            platform: navigator.platform || "unknown",
            screenWidth: window.screen.width,
    ...
    (0, import_react2.useEffect)(() => {
      if (pin.length === 4 && showPinEntry && !isLoading) {
        handlePinVerification();
      }
    }, [pin, showPinEntry, isLoading]);
    const checkDevice = async () => {
      ...
      try {
        ...
        const requestHeaders = {};
        if (existingHeader) {
          console.log("Using existing device header for cross-browser recognition");
          requestHeaders["X-Device-Header"] = JSON.stringify(existingHeader);
          if (!deviceKey && existingHeader.deviceId) {
            deviceKey = existingHeader.deviceId;
            (0, import_deviceKey2.storeDeviceKey)(deviceKey);
          ...
          if (deviceFingerprint) {
            console.log("No complete header found, using basic device fingerprint");
            try {
    ...
    const handlePinVerification = async () => {
      ...
      try {
        ...
        const requestHeaders = {};
        if (existingHeader) {
          console.log("Using existing device header for PIN verification");
          requestHeaders["X-Device-Header"] = JSON.stringify(existingHeader);
        }
        const deviceKey = await (0, import_deviceKey2.getStoredDeviceKey)();
        if (deviceKey) {
    ...
    const handleInitialSubmit = async () => {
      ...
      try {
        ...
        const deviceHeaderData = existingHeader || {
          deviceId: deviceKey,
          timestamp: Date.now(),
          deviceCharacteristics
        };
        if (existingHeader && existingHeader.userGuid && existingHeader.userHandle) {
          console.log("Using complete existing header with:", {
            deviceId: existingHeader.deviceId.substring(0, 10) + "...",
            userGuid: existingHeader.userGuid,
            userHandle: existingHeader.userHandle
          });
        ...
        if (loginMethod === "handle" && identifier.startsWith("@")) {
          console.log("Attempting fast authentication for handle:", identifier);
          try {
            const fastResponse = await axios_default2.post("fast_authenticate", {
              identifier
            }, {
              headers: {
                "X-CSRF-Token": window.csrfToken,
                "X-Device-Key": deviceKey,
                "X-Device-Header": JSON.stringify(deviceHeaderData)
              }
            });
            console.log("Fast authentication response:", fastResponse.data);
            if (fastResponse.data.status === "authenticated") {
              console.log("Fast authentication successful - high confidence match");
              (0, import_deviceKey2.storeDeviceSessionData)(fastResponse.data);
              ensureDeviceHeaderCreated(fastResponse.data);
              console.log(
                "AFTER AUTH SUCCESS - Device header content:",
                localStorage.getItem("superapp_device_header")
              );
              setFlowState("loginSuccess");
              setTimeout(() => {
                window.location.href = fastResponse.data.redirect_to || "/dashboard";
              }, 1e3);
              return;
            } else if (fastResponse.data.status === "needs_pin_verification") {
              console.log("Medium confidence match - offering PIN verification");
              setHandle(fastResponse.data.handle);
              setPhone(fastResponse.data.masked_phone);
              setWelcomeMessage(`Welcome back, ${fastResponse.data.handle}!`);
              setShowPinEntry(true);
              setPinAvailable(true);
              setExistingUserData({
                handle: fastResponse.data.handle,
                masked_phone: fastResponse.data.masked_phone,
                pin_available: true
              });
              setIsLoading(false);
              return;
            }
            console.log("Fast authentication not decisive, continuing with regular flow");
          } catch (err) {
            console.error("Fast authentication error:", err);
          }
        }
        if (loginMethod === "handle") {
          ...
          try {
            ...
            if (response.data.status === "authenticated") {
              console.log("Authentication successful - high confidence match");
              (0, import_deviceKey2.storeDeviceSessionData)(response.data);
              ensureDeviceHeaderCreated(response.data);
              setFlowState("loginSuccess");
              setTimeout(() => {
              ...
              return;
            } else if (response.data.status === "needs_pin_verification") {
              console.log("Medium confidence with PIN - offering PIN verification");
              setHandle(response.data.handle || "");
              setPhone(response.data.masked_phone);
              setWelcomeMessage(`Welcome back, ${response.data.handle}!`);
              setShowPinEntry(true);
              ...
              setFlowState("verification");
              setWelcomeMessage(`Welcome back, ${response.data.handle}!`);
              setPinAvailable(response.data.pin_available || false);
              setIsLoading(false);
              return;
              ...
              sessionStorage.setItem("account_already_exists", JSON.stringify({
                ...
                pin_available: response.data.pin_available || false
              }));
              setShowExistingAccountAlert(true);
              setAccountType("handle");
          ...
          try {
            console.log("Pre-checking phone:", identifier);
            ...
            console.log("Phone pre-check response:", checkResponse.data);
            if (checkResponse.data.exists) {
              const isRecognizedDevice = checkResponse.data.is_your_device;
              const deviceConfidence = checkResponse.data.device_confidence || "low";
              const confidenceScore = checkResponse.data.confidence_score || 0;
              ...
              if (checkResponse.data.exists && !isRecognizedDevice) {
                console.log("Phone exists on another device, showing Account Already Exists screen");
                sessionStorage.setItem("account_already_exists", JSON.stringify({
                  type: "phone",
                  phone: identifier,
                  handle: checkResponse.data.handle,
            ...
            console.log("Checking phone with main verification endpoint:", identifier);
            const response = await axios_default2.post("verify_login", {
              identifier,
              auth: { identifier }
            }, {
    ...
    const handlePhoneSubmit = async () => {
      ...
      try {
        ...
        if (existingHeader) {
          console.log("Using existing device header for phone verification");
          requestHeaders["X-Device-Header"] = JSON.stringify(existingHeader);
        } else {
          const deviceCharacteristics = {
            platform: navigator.platform || "unknown",
    ...
    const handleHandleSubmit = async () => {
      ...
      try {
        ...
        const requestHeaders = {};
        if (existingHeader) {
          console.log("Using existing device header for handle submission");
          requestHeaders["X-Device-Header"] = JSON.stringify(existingHeader);
        }
        const deviceKey = await (0, import_deviceKey2.getStoredDeviceKey)();
        if (deviceKey) {
    ...
    const handleVerificationSubmit = async () => {
      ...
      try {
        ...
        if (existingHeader) {
          console.log("Using existing device header for verification submission");
          requestHeaders["X-Device-Header"] = JSON.stringify(existingHeader);
        }
        if (deviceKey) {
          requestHeaders["X-Device-Key"] = deviceKey;
    ...
    const renderLoginOptions = () => /* @__PURE__ */ import_react2.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "w-full max-w-md space-y-8" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "text-center" }, /* @__PURE__ */ import_react2.default.createElement("h1", { className: "text-2xl font-bold mb-2" }, "Welcome to SuperApp"), /* @__PURE__ */ import_react2.default.createElement("p", { className: "text-gray-400" }, deviceNotRegistered ? "Login with an existing account or create a new one." : "Login or register to continue")), showExistingAccountAlert && /* @__PURE__ */ import_react2.default.createElement("div", { className: "flex flex-col items-center text-center space-y-6 py-4" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "w-16 h-16 rounded-full bg-teal-500 flex items-center justify-center" }, accountType === "handle" ? /* @__PURE__ */ import_react2.default.createElement("span", { className: "text-white text-2xl" }, "@") : /* @__PURE__ */ import_react2.default.createElement(User, { className: "w-8 h-8 text-white" })), /* @__PURE__ */ import_react2.default.createElement("div", { className: "space-y-2" }, /* @__PURE__ */ import_react2.default.createElement("h2", { className: "text-2xl font-bold" }, accountType === "handle" ? "This Handle Is Already Registered" : "Phone Number Already Registered"), /* @__PURE__ */ import_react2.default.createElement("p", { className: "text-gray-400" }, accountType === "handle" ? `The handle ${existingHandle || identifier} is already in use. Are you the owner of this account or would you like to choose a different handle?` : `This phone number is associated with an existing account. If this is your account, you can sign in. Otherwise, please use a different number.`)), /* @__PURE__ */ import_react2.default.createElement("div", { className: "space-y-3 w-full", id: "account-exists-buttons" }, /* @__PURE__ */ import_react2.default.createElement(
      "button",
      {
        id: "yes-account-btn",
      ...
      isLoading ? /* @__PURE__ */ import_react2.default.createElement("div", { className: "flex items-center justify-center" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" }), "Verifying...") : accountType === "handle" ? "Sign In With SMS" : "Sign In With SMS"
    ...
    const renderDeviceRegistration = () => {
      ...
      return /* @__PURE__ */ import_react2.default.createElement("div", { className: "min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "w-full max-w-md space-y-6" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "mb-8 flex justify-center" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "w-16 h-16 rounded-full bg-teal-500 flex items-center justify-center text-xl font-bold" }, existingUserData?.handle ? existingUserData.handle[0].toUpperCase() : identifier.startsWith("@") ? identifier[1].toUpperCase() : "S")), /* @__PURE__ */ import_react2.default.createElement("div", { className: "text-center" }, /* @__PURE__ */ import_react2.default.createElement("h2", { className: "text-2xl font-bold mb-2" }, isPhoneFlow ? "Phone Number Already Registered" : "This Handle Is Already Registered"), /* @__PURE__ */ import_react2.default.createElement("p", { className: "text-gray-400 mb-6" }, isPhoneFlow ? `This phone number is associated with an existing account. If this is your account, you can sign in. Otherwise, please use a different number.` : `The handle ${existingUserData?.masked_handle || maskHandle(identifier)} is already in use. Are you the owner of this account or would you like to choose a different handle?`)), /* @__PURE__ */ import_react2.default.createElement("div", { className: "space-y-4" }, /* @__PURE__ */ import_react2.default.createElement(
        "button",
        {
          ref: yesButtonRef,
          onClick: handleRegisterDevice,
        ...
        isLoading ? /* @__PURE__ */ import_react2.default.createElement("div", { className: "flex items-center justify-center" }, /* @__PURE__ */ import_react2.default.createElement("div", { className: "animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2" }), "Processing...") : /* @__PURE__ */ import_react2.default.createElement("span", { className: "flex items-center" }, isPhoneFlow ? "Sign In With SMS" : "Sign In With SMS", " ", /* @__PURE__ */ import_react2.default.createElement(ArrowRight, { size: 18, className: "ml-2" }))
      ), (pinAvailable || existingUserData && existingUserData.pin_available) && /* @__PURE__ */ import_react2.default.createElement(
        "button",
        {
          onClick: () => setShowPinEntry(true),
        ...
        /* @__PURE__ */ import_react2.default.createElement("span", { className: "flex items-center" }, "Sign In With PIN ", /* @__PURE__ */ import_react2.default.createElement(ArrowRight, { size: 18, className: "ml-2" }))
      ), /* @__PURE__ */ import_react2.default.createElement(
        "button",
        {
          ref: noButtonRef,
    ...
    return /* @__PURE__ */ import_react2.default.createElement(import_react2.default.Fragment, null, flowState === "checking" && renderLoadingState(), flowState === "deviceRegistered" && renderDeviceRegistered(), flowState === "loginSuccess" && renderLoginSuccess(), flowState === "deviceNotRegistered" && renderDeviceNotRegistered(), flowState === "loginOptions" && renderLoginOptions(), flowState === "handleEntry" && renderHandleEntry(), flowState === "phoneEntry" && renderPhoneEntry(), flowState === "deviceRegistration" && renderDeviceRegistration(), flowState === "verification" && renderVerificationStep(), flowState === "createHandle" && renderCreateHandle(), flowState === "handleSuggestions" && renderHandleSuggestions(), showPinEntry && renderPinEntry());
  };
