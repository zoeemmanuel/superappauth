(() => {
  ...
  var require_deviceKey = __commonJS({
    "app/javascript/utils/deviceKey.js"(exports, module) {
      ...
      var generateDeviceHeader2 = (deviceId, userGuid, userHandle) => {
        if (!deviceId || !userGuid) {
          console.warn("Missing required parameters for device header:", { deviceId, userGuid, userHandle });
          return null;
        }
        try {
          const headerData = {
          ...
          console.log(
            "Generated and stored device header for cross-browser recognition:",
            headerString.substring(0, 20) + "..."
          );
          return headerString;
        } catch (err) {
      ...
      var getDeviceFingerprint = async () => {
        console.log("getDeviceFingerprint called (stub) - using device key instead");
        return await getStoredDeviceKey2();
      };
      var getFingerprintComponents = () => {
        console.log("getFingerprintComponents called (stub) - using minimal data");
        return {
          installationId: sessionStorage.getItem("device_key") || "unknown",
      ...
      var storeDeviceSessionData = (data) => {
        ...
        if (data.device_header_data) {
          console.log(
            "Processing device_header_data from authentication response:",
            JSON.stringify(data.device_header_data).substring(0, 50) + "..."
          );
          const headerResult = generateDeviceHeader2(
            data.device_header_data.deviceId,
            data.device_header_data.userGuid,
            data.device_header_data.userHandle
          );
          if (headerResult) {
            console.log("Successfully stored device header in localStorage");
          } else {
            console.warn("Failed to store device header data from authentication response");
          }
        } else {
          console.log("No device_header_data in authentication response");
          if (data.device_key && data.guid && data.handle) {
            console.log("Creating header data from individual fields");
            generateDeviceHeader2(data.device_key, data.guid, data.handle);
          }
        }
      };
      ...
      module.exports = {
        ...
        // New device header functions
        generateDeviceHeader: generateDeviceHeader2,
        getDeviceHeader: getDeviceHeader2
      };
    }
  });
    ...
    "app/javascript/config/axios.js"() {
      ...
      instance.interceptors.request.use(async (config) => {
        try {
          ...
          const deviceHeader = (0, import_deviceKey.getDeviceHeader)();
          if (deviceHeader) {
            console.log("Setting request header X-Device-Header for cross-browser recognition");
            config.headers["X-Device-Header"] = deviceHeader;
          } else {
            console.log("No device header available to include in request");
          }
          ...
          if (response.data) {
            ...
            if (response.data.status === "authenticated") {
              ...
              if (response.data.device_header_data) {
                ...
                const headerData = (0, import_deviceKey.generateDeviceHeader)(
                ...
                if (headerData) {
                  ...
                  console.warn("Failed to store device header data");
                }
              } else {
                console.log("No device_header_data in authenticated response");
              }
            }
          }
        } catch (error) {
